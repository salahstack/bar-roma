...
const SignatureOfferings  = () => {
  return (
    <section className='section bg-white reveal-up'>
      <div className='container'>
        <div className='font-playfair text-center'>
          ...
          <h2 className='font-playfair text-5xl font-bold reveal-up'>Offerte Esclusive</h2>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-17'>
          {
            signatureOfferingsData.map(({ title, description, image, label, href, icon, category }, index) => (
              <SignatureOfferingsCard title={title} description={description} image={image} label={label} href={href} category={category} classes='reveal-up'  key={index} icon={icon}/>
            ))
          }
        </div>
      </div>
